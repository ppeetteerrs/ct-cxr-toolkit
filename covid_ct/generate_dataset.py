...
if __name__ == "__main__":
    ...
    if OUTPUT_LMDB:
        (Path("covid_ct/output") / "lmdb").mkdir(parents=True, exist_ok=True)
        writer = LMDBImageWriter(Path("covid_ct/output") / "lmdb")
    ...
    for idx, (subject, med_paths, loc_paths) in tqdm(
        ...
        lung_mask, body_mask, bones_mask = segment(med_sitk, med_np, seg_model=seg_model)
