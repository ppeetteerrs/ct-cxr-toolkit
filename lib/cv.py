...
from lib.utils import AnyArr, Arr8U, Arr32F, ArrB

CONTOUR_IDX = 0 if int(cv.__version__[0]) >= 4 else 1


def remove_border(
    img: AnyArr, axes: Optional[Collection[int]] = None, tol: float = 0
) -> AnyArr:
    """
    Removes border from an nd-array.

    Args:
        img (AnyArr): Input array
        tol (int, optional): Minimum brightness tolerance. Defaults to 50.

    Returns:
        AnyArr: Cropped image
    """
...
def project(
    ...
    *,
    ...
    Args:
        img_3d (Union[List[Arr32F], Arr32F]): 3D image or list of 2D images
    ...
    img_3d = remove_border(img_3d, remove_border_axes)
